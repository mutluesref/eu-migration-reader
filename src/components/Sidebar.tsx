...
export default function Sidebar({
  ...
  const copyRegNumber = async (docId: string, regNum: string) => {
    const ok = await copyToClipboard(regNum);
    if (ok) {
      setCopiedId(docId);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-4 py-3 border-b border-surface-100 dark:border-surface-700/50">
        <h2 className="text-[11px] font-semibold text-surface-400 dark:text-surface-500 uppercase tracking-wider">
          Documents
        </h2>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar py-2 px-2.5">
          ...
          <div className="mb-1">
            ...
            {showBookmarks && (
              <div className="px-1 pb-1">
                {bookmarks.map(
                  (b: { docId: string; articleNumber: string; label: string }, i: number) => (
                    <div
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-slate-500 dark:text-slate-400 truncate">
                          {getArticleTitle(b.docId, b.articleNumber)}
                        </div>
                      </div>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          removeBookmark(b.docId, b.articleNumber);
                        }}
        ...
        {recentArticles.length > 0 && !showBookmarks && (
          <div>
            <div className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-1 px-2">
              Recent
            </div>
            {recentArticles
              .slice(0, 5)
              .map((r: { docId: string; articleNumber: string }, i: number) => (
                <div
...
function renderArticleList(
  ...
  for (const article of doc.articles) {
    ...
    items.push(
      ...
      >
        {isActiveArt && (
          <span className="w-1 h-1 rounded-full bg-blue-500 flex-shrink-0 active-dot" />
        )}
        {hasNote && !isActiveArt && (
