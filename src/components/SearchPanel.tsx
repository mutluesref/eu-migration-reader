...
import { searchDocuments, type SearchFilters, type ContentType } from '../services/searchService';
...
function highlightSnippet(snippet: string, query: string): ReactNode[] {
  ...
  return parts.map((p, i) =>
    p.toLowerCase() === trimmed.toLowerCase() ? (
      <mark key={i} className="bg-yellow-100 text-slate-800 rounded-sm px-0.5 font-medium">
        {p}
      </mark>
    ) : (
      p
...
export default function SearchPanel({
  ...
  }, [debouncedQuery, documents, documentFilter, contentType]);
              ...
              {results.slice(0, 30).map((r, i) => (
                <div
                  ...
                  onClick={() =>
                    onResultClick(
                      r.documentId,
                      r.source === 'recital' ? 'recitals' : r.articleNumber,
                    )
                  }
                ...
                >
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${DOC_BADGE_COLORS[r.documentId] || 'bg-slate-100 text-slate-600'}`}
                    >
                      {r.documentName}
                    </span>
